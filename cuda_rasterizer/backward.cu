    ...
    int fb_width, int fb_height)
{
    ...
    cam.fb_width = unsigned(fb_width);
	...
	float* __restrict__ dL_dcolors)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            const float alpha = min(0.99f, con_o.w * G); // todo: fix gradient here
    ...
    int fb_width, int fb_height)
{
    ...
    preprocessCUDA<NUM_CHANNELS> << < (P + 255) / 256, 256 >> > (
		P, D, M,
		reinterpret_cast<const glm::vec3*>(means3D),
		radii,
		reinterpret_cast<const stroke::Cov3_f*>(cov3Ds),
		focal_x, focal_y,
		tan_fovx, tan_fovy,
		viewmatrix,
		opacities,
		...
		dL_dmean3D,
		dL_dcolor,
		reinterpret_cast<stroke::Cov3_f*>(dL_dcov3D),
		dL_dsh,
		dL_dscale,
		dL_drot,
        fb_width, fb_height);
